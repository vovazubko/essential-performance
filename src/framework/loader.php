 ...
 * @package    EssentialPerformanceFramework
...
use EssentialPerformanceFramework\Core\App;
...
try {
    spl_autoload_register(function ($class) {
        $loaders = [
            [
                'prefix' => 'EssentialPerformanceFramework\\',
                'directory' => __DIR__,
            ],
            [
                'prefix' => 'EssentialPerformance\\',
...
if (!function_exists('get_EssentialPerformance')) {
    /**
     * Get reference for application
     *
     * @return App
     */
    function get_EssentialPerformance()
    {
        static $app = null;

        if ($app === null) {
            $app = new App('EssentialPerformance');
        }

        return $app;
    }
} else {
    throw new \RuntimeException(sprintf('Global function "%s" has a name conflict', 'get_EssentialPerformance'));
}
